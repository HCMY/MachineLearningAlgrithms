...
def load_data():
	if os.path.exists('../resource/train.npy'):
		train = np.load('../resource/train.npy')
		return train

	domains360 = pd.read_csv('../resource/360.txt',
							header=None)[[1]]
	...
	domainsdga = pd.read_csv('../resource/dga-feed.txt', 
								names=['domain'], 
								header=None)
	domainsdga = domainsdga.dropna()
	domainsdga['label'] = [0]*domainsdga.shape[0]

	domain_normal = pd.read_csv('../resource/normal_domains.csv', 
							names=['domain'],
							header=None)
	...
	np.save('../resource/train.npy', train)
