...
class DecisionTree:
    ...
    def _grow(self, X, Y, cur_depth=0):
        ...
        if len(set(Y)) == 1:
            ...
            #如果是分类的话, 返回概率, 否则就是返回Y的值
            #一般, 这个Y的值, 在GBDT里面, 是负梯度拟合到的残差
        ...
        #选出最好的特征, 及其切分点, 然后根据切分点, 划分为左右2棵子树
        #这两颗子树, 各自对应不同的数据集
        #理论上, 被选中的特征的作用在这里结束, 但这个特征还是会参与后面的特征选择
        #但是理论上, 该特征发挥的效用低于其他的特征
        ...
        return Node(left, right, (feat, thresh))#保存这个分割点的值,以及其对应的特征
    ...
    def _impurity_gain(self, Y, split_thresh, feat_values):
        ...
        #不管是回归还是分类, 两边的loss都需要相加,作为最终的loss
